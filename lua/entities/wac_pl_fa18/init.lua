...
function ENT:CustomPhysicsUpdate(ph)
	...
	if IsValid(phys) and not self.disabled then
		if self.upMul>0.9 and self.rotorRpm>0.8 and phys:GetVelocity():Length() > 2500 and trace.HitPos:Distance( self:LocalToWorld(Vector(0,0,62)) ) > 50  and self:GetSequence() != gearup then
